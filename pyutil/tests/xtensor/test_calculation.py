...
def test_fillna():
    ...
    assert xt.fillna(fill_method="ffill") == XTensor([1, 1, 3, 3, 5])
    ...
    assert xt.fillna(fill_method="ffill") == XTensor([[[1, 2, 2], [np.NaN, 3, 4]],
                                                      [[5, 5, 7], [np.NaN, 100, 100]]])
    assert xt.fillna(fill_method="bfill", axis=1) == XTensor([[[1, 2, 4], [np.NaN, 3, 4]],
...
def test_shift(nested_ls):
    ...
    assert xt.shift(1, -1, fill_method="roll") == XTensor([[[3, 1, 2], [6, 4, 5]],
                                                           [[30, 10, 20], [60, 40, 50]]])
    target = XTensor([[[10, 10, 10], [10, 10, 10]], [[1, 2, 3], [4, 5, 6]]])
    ...
    assert shifted == XTensor([[[0.15, 1, 2], [0.45, 4, 5]], [[0.1, 10, 20], [0.4, 40, 50]]])
...
def test_diff(nested_ls):
    xt = XTensor(nested_ls)
    target = XTensor([[[np.NaN, 1, 1], [np.NaN, 1, 1]],
                      [[np.NaN, 10, 10], [np.NaN, 10, 10]]])
    assert xt.diff() == target
    target = XTensor([[[-3, -3, -3], [np.NaN, np.NaN, np.NaN]],
